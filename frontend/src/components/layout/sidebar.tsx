...
const CANDIDATE_NAV: NavEntry[] = [
  ...
  },
  {
    ...
    item: {
      ...
      href: ROUTES.candidate.upcoming,
      icon: CalendarClock,
    },
  },
  {
      ...
      children: [
        {
          ...
          href: `${ROUTES.candidate.results}?type=practice`,
          icon: FileCheck,
        },
        {
          label: "Assigned Interview Results",
          href: `${ROUTES.candidate.results}?type=assigned`,
          icon: FileCheck,
        },
      ],
    },
...
/**
 * Candidate sidebar. Persistent on ``lg+`` screens, drawer-style on mobile
 * with the "hamburger" trigger owned by ``<Header />`` (state shared via
 * ``useSidebarStore``).
 */
  ...
  return (
    <>
      ...
      <aside className="hidden w-64 shrink-0 border-r bg-card lg:block">
        <SidebarBody entries={CANDIDATE_NAV} />
      </aside>
      ...
      {isOpen ? (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div className="absolute inset-y-0 left-0 flex w-72 flex-col border-r bg-card shadow-xl">
            ...
            <SidebarBody entries={CANDIDATE_NAV} onNavigate={close} />
          </div>
        </div>
      ) : null}
    </>
